...
public class WeatherAPITest {
    ...
    public void buildRESTRequestTest() throws Exception {
        ...
        ).when(restClient).executeRequest(Mockito.any(HttpUriRequest.class));
        PowerMockito.doCallRealMethod().when(restClient).addParam(Mockito.anyString(),Mockito.anyString());
        PowerMockito.doCallRealMethod().when(restClient).execute(RestClient.RequestMethod.GET);
    ...
    public void GetWeatherAndDeserialiseResponseTest() throws Exception {
        ...
        Assert.assertEquals("Temp should match", "Clouds", weather.getWeather().get(0).getMain());
        Assert.assertEquals("Temp should match", WeatherResponse.ConditionCode.FEW_CLOUDS, weather.getWeather().get(0).getId());
        Assert.assertEquals("Temp should match", "few clouds", weather.getWeather().get(0).getDescription());
    }
